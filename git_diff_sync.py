...
def main(argv):
  ...
  parser.add_argument("source_repo", help="The path to the source repository.")
  parser.add_argument("destination_directory", help="The path to the destination directory.")
  parser.add_argument("--base-branch", default="main", help="The base branch for 'git diff' to determine changes. (default: main)")
  parser.add_argument("--verbose", action="store_true", help="Enable detailed output of file operations.")
  ...
  try:
    ...
    subprocess.run(["git", "pull"], cwd=source_repo, check=True, capture_output=True, text=True)
    ...
    if current_branch != args.base_branch:
      ...
      subprocess.run(["git", "checkout", args.base_branch], cwd=source_repo, check=True, capture_output=True, text=True)
      subprocess.run(["git", "pull"], cwd=source_repo, check=True, capture_output=True, text=True)

      # Go back to the original branch
      print(f"Checking out original branch '{current_branch}'...")
      subprocess.run(["git", "checkout", current_branch], cwd=source_repo, check=True, capture_output=True, text=True)
  ...
  result = subprocess.run(git_diff_command, cwd=source_repo, capture_output=True, text=True)
  ...
  if not operations:
    ...
    if not args.yes:
      proceed = input("Do you want to proceed with these operations? (y/n): ").lower()
      if proceed != 'y':
        print("Operation cancelled by user.")
        sys.exit(0)

    print("Performing file operations...")
    copy_success, copy_fail, delete_success, delete_fail, delete_skipped = 0, 0, 0, 0, 0
    ...
    for op, path in operations:
      ...
      if op == "copy":
        if not os.path.exists(source_path):
          print(f"Warning: Source file for copy does not exist, skipping: {source_path}")
          copy_fail += 1
          continue
        ...
        if os.path.isfile(destination_path):
          ...
          if args.verbose:
            print(f"Info: File to delete already gone, skipping: {destination_path}")
          delete_skipped += 1
